...
async def strcall(client, message):
    ...
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./CLeVeRMusic/assets/call.mp3"), stream_type=StreamType().pulse_stream)
        text="الناس القاعده في المكالمه ترغي:\n\n"
        ...
        text="االناس القاعده في المكالمه ترغي:\n\n"
        participants = await assistant.get_participants(message.chat.id)
        k =0
        for participant in participants:
            info = participant
